...
(
	id_ose int not null primary key auto_increment,
    jmeno varchar(10) not null,
    prijmeni varchar(10) not null
);

create table ose_zvi
(
	id_ose int not null,
    id_zvi int not null,
    primary key(id_ose,id_zvi)
);

alter table ose_zvi add constraint FK_ose_zvi foreign key (id_ose) references osetrovatel(id_ose);
alter table ose_zvi add constraint FK_zvi_ose foreign key (id_zvi) references zvire(id_zvi);
